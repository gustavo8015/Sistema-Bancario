...
class VentanaCajero(tk.Tk):
    ...
    def _construir(self):
        ...
        self.lbl_saldo.grid(row=1, column=0, columnspan=2, sticky="w", pady=(0, 10))

        ttk.Label(izq, text="Cuenta").grid(row=2, column=0, sticky="w")
        self.ent_cuenta = ttk.Entry(izq, width=12)
        self.ent_cuenta.insert(0, "1001")
        self.ent_cuenta.grid(row=2, column=1, sticky="w")

        ttk.Label(izq, text="Monto a retirar").grid(row=3, column=0, sticky="w")
        self.ent_monto = ttk.Entry(izq, width=12)
        self.ent_monto.insert(0, "100")
        self.ent_monto.grid(row=3, column=1, sticky="w")

        ttk.Label(izq, text="Modo de consistencia").grid(row=4, column=0, sticky="w", pady=(8, 0))
        self.modo = tk.StringVar(value="fuerte")
        ttk.Radiobutton(izq, text="Fuerte (bloqueo + 2PC)", variable=self.modo, value="fuerte").grid(row=5, column=0, columnspan=2, sticky="w")
        ttk.Radiobutton(izq, text="Eventual (replica local, propaga despues)", variable=self.modo, value="eventual").grid(row=6, column=0, columnspan=2, sticky="w")

        self.sincronizar = tk.BooleanVar(value=True)
        ttk.Checkbutton(izq, text="Retiro simultaneo: esperar al otro cajero", variable=self.sincronizar).grid(row=7, column=0, columnspan=2, sticky="w", pady=(8, 0))

        self.btn_retirar = ttk.Button(izq, text="RETIRAR", command=self.retirar)
        self.btn_retirar.grid(row=8, column=0, columnspan=2, sticky="ew", pady=(12, 4))
        ttk.Button(izq, text="Consultar saldo", command=self.consultar).grid(row=9, column=0, columnspan=2, sticky="ew", pady=2)
        self.btn_red = ttk.Button(izq, text="Simular caida de red de mi ciudad", command=self.alternar_red)
        self.btn_red.grid(row=10, column=0, columnspan=2, sticky="ew", pady=2)
        ttk.Button(izq, text="Reiniciar cuenta a 100", command=self.reiniciar).grid(row=11, column=0, columnspan=2, sticky="ew", pady=2)

        self.lbl_msg = ttk.Label(izq, text="", style="Msg.TLabel", wraplength=280, justify="left")
        self.lbl_msg.grid(row=12, column=0, columnspan=2, sticky="w", pady=(10, 0))
        ...
        self.lbl_coherente.grid(row=3, column=0, columnspan=2, sticky="w", pady=(4, 0))
    ...
    def retirar(self):
        try:
            ...
            return
        if self.sincronizar.get():
            self.lbl_msg.configure(text="Esperando al otro cajero...", foreground="#555")
        self._en_hilo({"op": "RETIRAR", "ciudad": self.ciudad, "cuenta": self.ent_cuenta.get(),
                       "monto": monto, "modo": self.modo.get(), "sincronizar": self.sincronizar.get()})
